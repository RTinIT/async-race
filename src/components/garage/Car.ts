...
class Car extends Component {
  ...
  constructor(parentNode: HTMLElement, car: CarType) {
    super(parentNode, null, 'div', 'car', `${car.id}`);

    this.selectBtn = new Buttons(this.node, () => {}, 'select-btn', 'select');

    this.removeBtn = new Buttons(this.node, () => {}, 'remove-btn', 'remove');

    this.startEngine = new Buttons(this.node, () => {}, 'start-btn', 'start');

    this.stopEngine = new Buttons(this.node, () => {}, 'stop-btn', 'stop');

    this.carName = new Component(this.node, null, 'span', '', '', `${car.name}`);
    this.svg = new Component(
      this.node,
      null,
      'div',
      'svg-container',
    ...
    <svg style="fill:${car.color}; width:80px; height:50px;">
